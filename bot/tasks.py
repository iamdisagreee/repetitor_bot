...
from bot.database import Debtor
from taskiq import Context, TaskiqDepends

from bot.database.taskiq_requests import give_information_for_day, give_lessons_for_day_students, give_lessons_for_day_teacher, \
    change_student_mailing_status, change_teacher_mailing_status
from bot.keyboards.taskiq_kb import create_confirmation_day_teacher_kb, create_confirmation_pay_student_kb, \
    create_notice_lesson_certain_time_student_ok, create_notice_lesson_certain_time_teacher_ok
from bot.lexicon.lexicon_taskiq import LEXICON_TASKIQ
from bot.services.services import give_week_day_by_week_date
from bot.services.services_taskiq import give_data_config_teacher, give_everyday_schedule, create_schedule_like_text, \
    check_is_30_minutes_between, create_scheduled_task, delete_unnecessary_tasks_teacher, give_dictionary_tasks_teacher, \
        ...
        for student_id, dict_week_date in list_students_id.items():
            ...
            for week_date, lessons_day in dict_week_date.items():
                ...
                for index, lesson_day in enumerate(lessons_day):
                    if len(lessons_day) == 1 and lesson_day.student_mailing_status == 0:
                        ...
                        break
                    # print("LEN(lessons_day)=", len(lessons_day))
            ...
            for student_id, dict_week_date in dict_students.items():
                for week_date, lessons_day in dict_week_date.items():
                    ...
                    for index, lesson_day in enumerate(lessons_day):
                        ...
                        if lesson_day.teacher_mailing_status == 1 and index != 0 \
                        ...
                        only_change = False
                        if not flag_status and index != 0 and \
                                lessons_day[index - 1].teacher_mailing_status == 1 and \
                                check_is_30_minutes_between(lessons_day[index - 1].lesson_start,
                                                            lessons_day[index].lesson_start):
                            only_change = True
                        ...
                        if not only_change or flag_status:
                            ...
                            await create_scheduled_task(task_name='notice_lesson_certain_time_teacher',
                                                        ...
                                                        schedule_id=f'b_l_t_{teacher_id}_{week_date}'
                                                                    f'_{lesson_day.lesson_start}',
                                                        until_hour=lesson_day.student.teacher.until_hour_notification,
                                                        until_minute=lesson_day.student.teacher.until_minute_notification,
                                                        lesson_start=lesson_day.lesson_start,
                                                        week_date=week_date
                                                        )
